...
test("Universe workspace is packaged for authenticated Platform and contextual public Lib routes", async () => {
  const [html, app, worker, build, shell, canonical] = await Promise.all([
    read("apps/universe/index.html"), read("shared/universe-app.js"), read("worker/entry.js"), read("scripts/build.mjs"), read("shared/shell.js"), read("shared/product-canonical.js"),
  ]);
  ...
  assert.match(worker, /const universeInfo = info\.surface === "lib" \? \{ \.\.\.info, app: "worlds" \} : info/);
  assert.match(worker, /\["platform", "lib"\]\.includes\(info\.surface\)/);
...
test("public Universe mode follows the routing query and canonical Lib projection", async () => {
  const [app, worker, canonical] = await Promise.all([read("shared/universe-app.js"), read("worker/entry.js"), read("shared/product-canonical.js")]);
  ...
  assert.match(canonical, /searchParams\.set\("mode", "public"\)/);
  assert.match(canonical, /canonicalOrigin = "https:\/\/lib\.idol\.id"/);
});
...
test("Universe presentation names exact non-semantic boundaries", async () => {
  const [html, app, manifestBuild, platformEntry] = await Promise.all([read("apps/universe/index.html"), read("shared/universe-app.js"), read("scripts/build.mjs"), read("shared/platform-universe-entry.js")]);
  for (const source of [html, app, platformEntry]) assert.match(source, /one semantic universe|operational[- ](?:view|projection)/i);
...
test("Universe documentation and additive migration are present", async () => {
  const [docs, migration, scopes] = await Promise.all([read("content/docs/universe.md"), read("migrations/0006_universe_views.sql"), read("shared/platform-auth.js")]);
