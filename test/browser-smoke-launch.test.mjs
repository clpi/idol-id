...
test("the real-Chrome orchestrator allocates ephemeral CDP ports and preserves bounded diagnostics", async () => {
  ...
  assert.match(runner, /IDOL_HOME_BROWSER_DEBUG_PORT/);
  ...
  assert.doesNotMatch(workflow, /^\s*node scripts\/(?:browser-smoke|live-mcp-browser-smoke|home-browser-smoke)\.mjs\s*$/m);
});
