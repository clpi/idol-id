...
function localRepositoryPath(pathname) {
  return pathname === "/repo"
    || pathname === "/repo/"
    || pathname.startsWith("/repo/observation/")
    || pathname.startsWith("/repo/scaffold/")
    || pathname.startsWith("/v1/repository/browser/")
...
export async function handle(request, env, dependencies = {}) {
  ...
  if ((url.pathname === "/install" || url.pathname === "/install.sh") && url.hostname === "idol.id") {
    return asset(env, request, "/content/install.sh", { immutable: false });
  }
  if (url.pathname === "/install.ps1" && url.hostname === "idol.id") {
