...
export const hostMap = Object.freeze({
  ...
  "book.idol.id": { app: "book", surface: "book", origin: true },
...
function assetFound(response) { return response.ok || response.status === 304; }
async function readJsonAsset(env, request, pathname) {
  ...
  try { return { value: await response.json() }; }
  catch { return { response: json({ error: "runtime projection is invalid JSON", path: pathname }, { status: 500 }) }; }
}
function validRuntimeAuthority(value) {
  return Boolean(value && typeof value.language?.commit === "string" && value.language.commit.length > 0 && typeof value.native?.commit === "string" && value.native.commit.length > 0 && typeof value.language?.source_law?.sha256 === "string" && value.language.source_law.sha256.length > 0);
}
async function loadRuntimeAuthority(env, request) {
  const loaded = await readJsonAsset(env, request, "/runtime/authority.json");
  if (loaded.response || !validRuntimeAuthority(loaded.value)) return { response: json({ error: { code: "RUNTIME_AUTHORITY_UNAVAILABLE", message: "immutable runtime authority projection is unavailable or invalid" } }, { status: 503 }) };
  return { value: loaded.value };
}
async function appShell(env, request, app) { return asset(env, request, `/apps/${app}/index.html`, { html: true }); }
...
function originless(info) { return info.origin === false; }
function noOrigin(info) { return json({ error: "dynamic origin unavailable on this surface", surface: info.surface }, { status: 404 }); }
...
async function worldTransport(request, env, pathname) {
  ...
  if (request.method === "POST" && pathname === "/v1/world/import-plan") {
    ...
    const loaded = await readJsonAsset(env, request, "/runtime/foreign.json");
    if (loaded.response) return loaded.response;
    try { return json(planForeignImport(parseImportRequest(input), loaded.value)); }
    catch (error) { return json({ error: error.message }, { status: error instanceof RangeError ? 422 : 400 }); }
  }
  return null;
}
...
export async function handle(request, env, dependencies = {}) {
  ...
  if (["/__idol/version", "/__idol/health", "/config.js"].includes(url.pathname)) {
    ...
    const identity = { commit, authority: authority.language.commit, native_authority: authority.native.commit, source_law: authority.language.source_law.sha256, app: info.app, surface: info.surface };
  ...
  if (info.surface === "platform" && /^\/ide(?:\/|$)/.test(url.pathname) && isNavigation(request, url.pathname)) return appShell(env, request, "ide");
  if (info.surface === "lib" && /^\/(?:atlas|world)(?:\/|$)/.test(url.pathname) && isNavigation(request, url.pathname)) return appShell(env, request, "worlds");
  if (info.surface === "lib" && /^\/universe(?:\/|$)/.test(url.pathname) && isNavigation(request, url.pathname)) return appShell(env, request, "universe");
