...
function slug(value) {
  return String(value || "not-published")
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9-]+/g, "-")
    .replace(/^-+|-+$/g, "") || "not-published";
}
...
export function semanticTokenClass(token) {
  ...
  if (token.published_source_face || (token.binding?.status === "published" && token.source_face !== "not-published")) {
    classes.push(`sf-${slug(token.source_face)}`);
  }
...
export function renderSemanticTokens(tokens, source) {
  ...
  for (const token of tokens) {
    ...
    const title = token.binding?.status === "ambiguous"
      ? `ambiguous compiler binding · ${semantic}`
      : semantic;
...
export class SemanticSourceView {
  ...
  setSource(source) {
    this.textarea.value = String(source);
  }

  getSource() {
    return this.mode === "edit" ? this.textarea.value : (this.bundle?.source || this.textarea.value);
  }
  ...
  setHighlights(indexes = []) {
    this.highlights = new Set(indexes.filter(Number.isInteger));
    this.#paint();
  }
